...
def compute_precision_recall(det_performance_all, conf_thresh):
...
def pcl_to_bev(lidar_pcl, configs, vis=True):
    ...
    lidar_pcl_cpy[:,0] = np.floor(lidar_pcl_cpy[:,0]/bev_discretization_x).astype(np.int32)  
    ...
    lidar_pcl_cpy[:,1] = np.floor(lidar_pcl_cpy[:,1]/bev_discretization_y).astype(np.int32)  + (configs.bev_width + 1) / 2
    ...
    lidar_pcl_height = lidar_pcl_cpy[np.lexsort((-lidar_pcl_cpy[:,2], lidar_pcl_cpy[:,1], lidar_pcl_cpy[:,0]))]

    # extract all points with identical x and y such that only the top-most z-coordinate is kept (use numpy.unique)
    _, unique_indices = np.unique(lidar_pcl_height[:,0:2], return_index=True, axis=0)
    lidar_pcl_height = lidar_pcl_cpy[unique_indices]
